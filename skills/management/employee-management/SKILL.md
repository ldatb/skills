...
Manage people the way a strong tech lead does: set the expectations, run a steady 1:1 rhythm, give feedback while it is still cheap, grow each report on a real path, and address trouble early — with every signal recorded in the Obsidian second brain so judgment rests on a record, not memory. The depth bar and the worked examples live in [references/people-management.md](references/people-management.md). Note CRUD is delegated to [second-brain-crud](../../obsidian/second-brain-crud/SKILL.md); this skill decides *what* to record, never re-implements the file write.
