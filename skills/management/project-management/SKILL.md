...
Note CRUD is not this skill's job. Reads, writes, links, and frontmatter go through [second-brain-crud](../../obsidian/second-brain-crud/SKILL.md); this skill decides *what* the project record must contain and *when* the state changes.
...
4. **Log risks and decisions as they surface.** Record each material risk with a likelihood, an impact, and an owner; capture each material decision in a decision note linked from the project via [second-brain-crud](../../obsidian/second-brain-crud/SKILL.md). A risk without an owner does not count as logged. The step is done when every open risk has an owner and every decision since the last review is linked.
...
6. **Correlate the project in the vault.** Link the project note to the people, companies, and commitments it touches via [second-brain-crud](../../obsidian/second-brain-crud/SKILL.md), so a query across the vault surfaces this project beside related work. The step is done when the project links to at least its owner and one company-or-commitment note, and the link resolves.
