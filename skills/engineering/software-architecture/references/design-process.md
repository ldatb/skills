...
another; the job is to make the trade deliberate and to emit it as an artifact.

This reference is the depth bar for the seven-step procedure in SKILL.md. The
decision procedure below expands those steps, the menus that follow supply the
options each step picks from, and the worked example runs the whole funnel end to
end. Cloud-specific moves live in [cloud architecture](cloud-architecture.md); the
output formats live in [C4 diagrams](diagrams-c4-mermaid.md) and
[ADR and docs](adr-and-docs.md).

## Decision procedure

The procedure is a funnel from problem to emitted artifact:
...
1. **Clarify requirements and constraints.** Functional scope answers "what must it
   ...
   deadline, compliance, data residency, and the stack already in production. A design
   without numbers is fiction, so an estimate of load and storage belongs on the page
   before any box is drawn.

2. **Identify the dominant quality attributes.** Two or three attributes decide the
   shape; the rest are satisfied, not optimized. A bank ranks consistency and security
   above latency; a social feed ranks the reverse. The ranking, with a target per
   attribute, is the spine the rest of the design hangs from.

3. **Sketch components and responsibilities.** Each component owns one responsibility
   and a slice of the data. The test of a good boundary: the things that change
   together live together, and the things that vary independently sit apart.
...
5. **Place it on infrastructure and define contracts.** Each component maps to a
   building block — a process, a managed service, a self-hosted server — per the
   [build-vs-managed call](cloud-architecture.md#build-vs-managed). Each boundary then
   names a call style, a payload shape, and a failure behavior. A contract is the
   promise one component makes to another; an unstated contract is a future outage.
...
7. **Emit the artifacts.** The [C4 diagrams](diagrams-c4-mermaid.md) and an
   [ADR](adr-and-docs.md#the-adr-template) leave the exercise. The artifacts are the
   deliverables that survive the meeting.
...
PACELC); cost fights everything; evolvability fights the speed of the first release.
Naming which attribute loses is the architecture.

## Sketch components and data

A component owns one responsibility and a slice of the data. Two tests separate a
real boundary from a line drawn for tidiness:

- **Cohesion** — the things that change together live in one component. A field that
  always changes alongside another belongs beside it, not across a network call.
- **Coupling** — the things that vary independently sit in different components. A
  reporting workload that scales on a different curve than the write path earns its
  own seam.

Model the data before the datastore. Name the entities, their keys, their
relationships, and — the deciding input — the **access patterns**: the exact reads
and writes the running system issues. The access patterns select the storage
technology; the storage technology never selects the access patterns. The C4 levels
that render this sketch live in [C4 diagrams](diagrams-c4-mermaid.md#the-c4-levels).
...
- **Big design up front.** Months of diagrams modeling requirements that have not been
  validated, obsolete before the first deploy. Counter: design the first increment
  fully, sketch the rest, and leave seams to extend.
...
- **Accidental distributed monolith.** Services split on paper but coupled in
  practice — synchronous call chains, a shared database, one deploy that must ship them
  together. Worst of both worlds: distribution's cost without its independence.
...
A walk through the funnel at the container level. The diagrams this example emits are
drawn in [C4 diagrams](diagrams-c4-mermaid.md#worked-example-the-url-shortener); the
decision it records is in [ADR and docs](adr-and-docs.md#worked-adr).

### 1. Requirements and constraints

- **Functional.** Shorten a long URL to a short code; redirect a short code to its
  long URL; report click counts per code.
- **Constraints.** Small team, cloud budget modest, public-facing, no data-residency
  restriction.
...
Consistency ranks low: a newly created code may take a second to be globally readable
without harm.
...
- `POST /shorten {url}` → `{code}` — synchronous; rejects an invalid or unreachable URL
  at the boundary with a 4xx.
...
- **Key-value store over relational.** The dominant access pattern is a point lookup by
  code; joins and ad-hoc queries are absent. Relational would serve correctly but
...
- **Async click counting over synchronous increments.** A synchronous counter write on
  every redirect doubles the write path and the latency; the queue keeps the redirect
  fast. Cost: counts are eventually consistent — acceptable per the ranking.

### 7. Emitted artifacts

The deliverables that leave this exercise: the System Context and Container diagrams in
[C4 diagrams](diagrams-c4-mermaid.md#worked-example-the-url-shortener), and ADR-001 in
[ADR and docs](adr-and-docs.md#worked-adr). Both are checkable — the Container diagram
gives every container one responsibility, and the ADR carries two rejected alternatives
and one named negative consequence.
