...
5. **Gate the change, then write the result back to state.** Run the named gate command; a red gate reverts the item to open, restores that item's changed files to their pre-iteration state from version control (`git restore <files>` on the loop's own unshared branch) so the next attempt starts clean rather than stacking on a half-done edit, and increments the stall count; a green gate moves the item to done and resets the stall count to zero. The step ends once the gate result is recorded, the working tree is clean of the failed item's edits, and `handoff/state.json` reflects the new open set, counts, and spend.
