...
follow-up. Craft decides the reply rate far more than the offer does, so the work
is precision, not volume. This reference is the depth behind the cold-outbound
lane of the outreach skill.

**Drafting only.** The skill drafts emails and sequences. Sending is an external
mutation against real recipients and a sender reputation, so a human approves and
sends every message. Nothing here authorizes an automatic send, and nothing here
schedules one.
...
2. **Opener / personalization** — earns the next sentence. One line proving the
   email was written for this person: a recent hire, a launch, a podcast quote, a
   number from their site. The opener is a fact about *them*, never a compliment
   ("love what you're building") and never a template variable left raw.
...
4. **Value** — one sentence on the outcome, not the feature. State the result a
   ...
   automation platform".
5. **Single CTA** — one ask, low-friction, easy to say yes to. A yes/no interest
   check ("worth a look?") or a soft time ask ("open to 15 minutes next week?").
   One CTA per email is a hard rule (see below).
...
| **Segment** | One template per tightly-defined segment; opener references a trait the whole segment shares (industry, stack, role, recent event) | Minutes per segment | Mid ACV; the segment is narrow enough that one trigger holds true for every recipient |
...
- **Attachments** — none. A cold email with an attachment is filtered hard, and
  a file from a stranger is often unsafe to open.
...
- **Broken merge tags** — a raw "Hi {first_name}," is the loudest "this is
  automated" tell and tanks reply rates.
...
One email, one ask. Each additional CTA splits the reader's attention and lowers
the odds of any action — a documented decision-paralysis effect. "Reply, or book
a call, or check out our pricing page, or follow us" is four asks and zero
clarity.

Pick the single lowest-friction next step the deal stage justifies:

- A cold first touch takes an interest check ("worth a quick look?"). Asking for
  30 minutes from a stranger is too big a first ask.
- A warm or replied contact takes the meeting ask, with a concrete window.

The CTA is a question the reader answers in one word or one click. A CTA that
forces the reader to think about *how* to respond is friction.

## The follow-up sequence

The single most common cause of a dead campaign is no follow-up. A large share
of replies arrive on the second through fourth touches, not the first. A sequence
is not nagging when each touch adds a new angle.
...
| 2 | +3 | Short bump: a new angle or a one-line proof point, never "just bumping this" |
| 3 | +7 | A different value angle — a case study, a resource, a different pain |
...
- Stop on a reply, on an opt-out, or after the break-up. Four to five touches is
  the normal ceiling; more reads as harassment and risks complaints.
...
A draft trips a red flag when any of these holds true:
...
- Two or more links, an attachment, or a spam-trigger word survived the check.
- More than one CTA, or a first-touch CTA asking for a large time commitment.
...
her language; value is one outcome with a number; one soft CTA; compliant footer.
~70 words, zero links, plain text.
...
sequence ends here; keeps the opt-out. The break-up email often pulls the highest
reply rate of the three.
...
- [the review lenses](../../../engineering/code-review/references/review-lenses.md) — the depth bar for judging a finished draft against what it must deliver.
